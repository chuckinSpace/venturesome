  ...
  try {
    if (
      ...
      await firebase.updateContact(
        contactId,
        objToSend,
        `updating contact with ${objToSend} on update firestore`
      );
    }
    //always update name
    const nameObj = await monday.getName(itemId);
    const contactId = await firebase.getContactId(itemId);
    if (nameObj.name !== null && nameObj.name !== "") {
      await firebase.updateContact(
        contactId,
        nameObj,
        `updating contact with ${nameObj.firstName} ${nameObj.lastName} on always update name`
      );
      res.send({ message: "success" });
    } else {
      res.send({ message: "success" });
